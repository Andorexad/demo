...
class StringTracker {
    ...
    typealias StringObservation = (lastSeen: Int64, count: Int64, position: CGRect)
    ...
    func logFrame(results: [VNRecognizedTextObservation]) {
        ...
        for result in results {
            let position = result.boundingBox
            var removedString = result.topCandidates(1).first!.string
			...
			if seenStrings[removedString] == nil {
                seenStrings[removedString] = (lastSeen: Int64(0), count: Int64(-1), position: position)
			}
			seenStrings[removedString]?.lastSeen = frameIndex
			seenStrings[removedString]?.count += 1
            seenStrings[removedString]?.position = position
//			print("Seen \(removedString) \(seenStrings[removedString]?.count ?? 0) times")
		}
